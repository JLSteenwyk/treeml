...
def extract_package_version() -> str:
    text = read_text(ROOT / "treeml" / "version.py")
    match = re.search(r'__version__\s*=\s*"([^"]+)"', text)
    if not match:
        raise ValueError("Could not parse __version__ from treeml/version.py")
...
def main() -> int:
    ...
    if package_version != changelog_version:
        mismatches.append(
            f"treeml/version.py ({package_version}) != CHANGELOG.md ({changelog_version})"
        )
